...
def show_images(original, mask, masked_image, means, stds, reconstructed, n=15):
    ...
    plt.tight_layout()
    plt.savefig('./convnextv2-masked-auto-encoder-reconstruction.png')  # Save the plot as an image
...
if __name__=='__main__':
    ...
    plt.savefig('./convnextv2-masked-auto-encoder.png')  # Save the plot as an image
    ...
    with torch.no_grad():
        for (inputs, means, stds), _ in val_loader:
            ...
            show_images(inputs, mask, masked_inputs, means, stds, outputs)
            break
